...
def evolve(init,nchild,Eval,rate_mutation, Iterations):
    ...
    while (igen< Iterations):
        ...
        gen_curr, fit_curr = str(data[0, list(data[1]).index(min(data[1]))]), float(min(data[1]))
        print(fit_curr, fit_best)
        if float(fit_curr) <= fit_best:
            gen_best = gen_curr
            fit_best = fit_curr
        igen     = igen + 1 
        print("igen=%5i fit=%13.5e gen=%s" % (igen,float(fit_curr),gen_curr))
    return igen, gen_best, fit_curr

...
function create_sequence(SeqOrder)
    ...
    for i in 1:2:nloop
        if SeqOrder[i] == 1
            add_raman_cool(ax_t, ax_f1_amp,ax_f2_amp,  true, false, 1, -1*SeqOrder[i+1])
        elseif SeqOrder[i] == 2
            add_raman_cool(r_t, r_f1_amp, r_f2_amp, true, false, 2, -1*SeqOrder[i+1])
        elseif SeqOrder[i] == 3
            add_raman_cool(r_t, r_f1_amp, r_f2_amp, true, false, 3, -1*SeqOrder[i+1])
        end 

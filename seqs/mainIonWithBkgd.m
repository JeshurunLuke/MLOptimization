...
tUVShtrOnDelay = 80e-3; %was 40e-3
tUVShtrSkip = 40e-3;
tUVShtrMinOn = 40e-3;
% For more info see comments in TTLMgr
...
function background10Hz(s, len)
    ...
    while s.totalTime() < len
        ...
        if cycleStart > ionStartBG && cycleStart < ionEndBG
            s.addBackground(@IonSyncProcBkgd); %% Shorter than 100ms
        end
        s.addStep(onTime) ...
            .add('TTLbkgd', 1);
